...
<body class="bg-blue-900 text-blue-100 font-sans h-screen flex flex-col justify-between">

    <header class="bg-blue-800 p-4 shadow flex justify-between items-center border-b border-blue-700">
        <div>
            <h1 class="text-xl font-bold text-teal-400">⚡Computer engineering group 6</h1>
            <!-- <p class="text-xs text-blue-400">FastAPI + PostgreSQL Chat Engine</p> -->
        </div>
        <div class="bg-blue-700 px-3 py-1.5 rounded-full flex items-center gap-2 border border-blue-600">
    ...
    <footer class="bg-blue-800 p-4 border-t border-blue-700">
        <div class="max-w-3xl mx-auto flex flex-col sm:flex-row gap-2">
            <input id="usernameInput" type="text" placeholder="Your username..." 
                   class="bg-blue-700 text-white px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 sm:w-1/4">
            <div class="flex-1 flex gap-2">
                <input id="messageInput" type="text" placeholder="Type a message..." disabled
                       class="flex-1 bg-blue-700 text-white px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 disabled:opacity-50">
    ...
    <script>
        ...
        function appendMessage(sender, text, type) {
            ...
            if (type === 'system') {
                msgDiv.className = "text-center text-xs text-blue-400 my-2 italic";
                msgDiv.innerText = text;
            } else {
                const isMe = sender === myUsername;
                msgDiv.className = `flex flex-col ${isMe ? 'items-end' : 'items-start'}`;
                msgDiv.innerHTML = `
                    <span class="text-xs text-blue-400 mb-1">${sender}</span>
                    <div class="${isMe ? 'bg-teal-500 text-blue-900 font-medium' : 'bg-blue-700 text-white'} px-4 py-2 rounded-2xl max-w-xs shadow">
                        ${text}
                    </div>
                `;
            }
    ...
    try:
        while True:
            ...
            if USE_DB:
                ...
                messages.append({"sender": username, "text": data})
